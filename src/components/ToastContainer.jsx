...
const Toast = ({ message, type = 'info', duration = 5000, onClose }) => {
  ...
  return (
    ...
    >
      <div className={`${colors[type]} rounded-ios-lg shadow-ios-xl p-4 flex items-center space-x-3`}>
...
const ToastContainer = () => {
  ...
  useEffect(() => {
    const handleRecurringExecuted = (event) => {
      const { recurring, transaction } = event.detail;
      const message = `${recurring.type === 'deposit' ? '✅' : '💸'} ${recurring.description}: ${formatCurrency(recurring.amount)}`;
      
      addToast(message, recurring.type === 'deposit' ? 'success' : 'info');
    };
  ...
  return (
    <>
      {toasts.map((toast, index) => (
        <div key={toast.id} style={{ top: `${80 + index * 80}px` }} className="absolute">
          <Toast
